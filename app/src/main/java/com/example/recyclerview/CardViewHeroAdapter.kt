...
class CardViewHeroAdapter(private val listHero: ArrayList<Hero>)
    ...
    override fun onBindViewHolder(holder: CardViewViewHolder, position: Int) {
       ...
       holder.btnFavorite.setOnClickListener{
           Toast.makeText(holder.itemView.context, "Favorite" + listHero[holder.adapterPosition].name,
               Toast.LENGTH_SHORT).show()
       }
        holder.btnShare.setOnClickListener{
            Toast.makeText(holder.itemView.context, "Share" + listHero[holder.adapterPosition].name,
                Toast.LENGTH_SHORT).show()
        }
